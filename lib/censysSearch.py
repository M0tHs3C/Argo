...
class censysSearch:
        ...
        if censys_list == []:
            ...
            try:
                ...
                try:
                    ...
                    menuBuilder.buildMenu(selectionArray=menuSelection,title="Censys search")
                    try:
                        selection = int(input('[-]Choose an option: '))
                        if selection == 1:
                            menuBuilder.buildMenu(selectionArray=cameraSelection,title="Censys camera search")
                            query = queryBuilder.QueryBuilder.CamQueryBuilderCensys(self)
                        elif selection ==2:
                            menuBuilder.buildMenu(selectionArray=vpnSelection, title="Censys vpn search")
    ...
    def samipGathCensys(self):
        ...
        try:
            for q in query:
                for record in censys.ipv4.CensysIPv4(api_id=uid, api_secret=secret).search(q):
                    ip = record['ip']
                    port = record['protocols']
                    port_raw = port[0]
                    port = re.findall(r'\d+', port_raw)
